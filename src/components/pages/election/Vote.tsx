...
import { IPartyEntity, ICandidateEntity, IElectionEntity } from "@/types/model";
import { Table, Radio, Row, Col, Button, Checkbox } from 'antd';
...
const ElectionVotePage: SFC<{}> = () => {
  ...
  return (
    <PageTemplate header={<Header />}>
      ...
      <h2>Interaktiver Stimmzettel für die {election.type} im {election.text}</h2>
      <div>
        <Row >
          <Col span={12}>
            <Radio.Group id="radioFirstVote">
              <Table bordered pagination={false} columns={columnsFirst} dataSource={candidates.map(candidatesData => {
                return {
                  key: candidatesData.id_candidate,
                  candidate: (candidatesData.last_name + " " + candidatesData.first_name + " " + parties.map(partiesData => {
          ...
          <Col span={12}>
            <Radio.Group id="radioSecondVote">
              <Table bordered pagination={false} columns={columnsSecond} dataSource={parties.map(partiesData => {
                return {
        ...
        <Row>
          <Col>
            <Button type="primary" onClick={() => {
              document.getElementById("radioFirstVote");
